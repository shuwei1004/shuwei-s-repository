...
-- 功能函数
-- 速度滑块
local walkSpeed = makeBaseDragUI(UDim2.new(.2, 0, .4, 0), UDim2.new(.65, 0, .25, 0), "速度")
walkSpeedSlider = makeSlider(walkSpeed, UDim2.new(1, 0, .2, 0), UDim2.new(0, 0, .4, 0))
local walkSpeedLabel = makeLabel(walkSpeed, UDim2.new(1, 0, .2, 0), UDim2.new(0, 0, .2, 0), "速度: 16", BrickColor.new("Royal purple").Color, Color3.new(0, 0, 0), .8)
local dragging = false
local pos = mouse.X
...
mouse.Move:connect(function()
	if dragging == true then
		...
		walkSpeedLabel.Text = "速度: "..tostring(walkSpeed)
	end
end)
-- 跳跃力滑块
local jumpPowerF = makeBaseDragUI(UDim2.new(.2, 0, .4, 0), UDim2.new(.65, 0, .3, 0), "跳跃力")
jumpPowerSlider = makeSlider(jumpPowerF, UDim2.new(1, 0, .2, 0), UDim2.new(0, 0, .4, 0))
local jumpPowerLabel = makeLabel(jumpPowerF, UDim2.new(1, 0, .2, 0), UDim2.new(0, 0, .2, 0), "跳跃力: 16", BrickColor.new("Royal purple").Color, Color3.new(0, 0, 0), .8)
local dragging = false
local pos = mouse.X
local jumpPower = 50
...
mouse.Move:connect(function()
	if dragging == true then
		...
		jumpPowerLabel.Text = "跳跃力: "..tostring(jumpPower)
	end
end)
-- 透视功能
local Esp = makeBaseDragUI(UDim2.new(.2, 0, .45, 0), UDim2.new(.65, 0, .35, 0), "玩家和物品透视")
local targetLabels = {}
local itemLabels = {}
...
local playerESP = makeButton(Esp, UDim2.new(1, 0, .25, 0), UDim2.new(0, 0, 0, 0), "玩家透视 关闭", BrickColor.new("Royal purple").Color, Color3.new(0, 0, 0), .4)
local arrowESP = makeButton(Esp, UDim2.new(1, 0, .25, 0), UDim2.new(0, 0, .25, 0), "箭矢透视 关闭", BrickColor.new("Royal purple").Color, Color3.new(0, 0, 0), .4)
local RokaESP = makeButton(Esp, UDim2.new(1, 0, .25, 0), UDim2.new(0, 0, .5, 0), "Roka透视 关闭", BrickColor.new("Royal purple").Color, Color3.new(0, 0, 0), .4)
local vampESP = makeButton(Esp, UDim2.new(1, 0, .25, 0), UDim2.new(0, 0, .75, 0), "吸血鬼面具透视 关闭", BrickColor.new("Royal purple").Color, Color3.new(0, 0, 0), .4)
playerESP.MouseButton1Click:connect(function()
	if findPlayers == false then
		findPlayers = true
		playerESP.Text = "玩家透视 开启"
	else
		findPlayers = false
		playerESP.Text = "玩家透视 关闭"
	end
end)
arrowESP.MouseButton1Click:connect(function()
	if findArrows == false then
		findArrows = true
		arrowESP.Text = "箭矢透视 开启"
	else
		findArrows = false
		arrowESP.Text = "箭矢透视 关闭"
	end
end)
RokaESP.MouseButton1Click:connect(function()
	if findRokas == false then
		findRokas = true
		RokaESP.Text = "Roka透视 开启"
	else
		findRokas = false
		RokaESP.Text = "Roka透视 关闭"
	end
end)
vampESP.MouseButton1Click:connect(function()
	if findMasks == false then
		findMasks = true
		vampESP.Text = "吸血鬼面具透视 开启"
	else
		findMasks = false
		vampESP.Text = "吸血鬼面具透视 关闭"
	end
end)
-- 关于
local credits = makeBaseDragUI(UDim2.new(.2, 0, .45, 0), UDim2.new(.65, 0, .4, 0), "关于")
local maker = makeLabel(credits, UDim2.new(1, 0, 1, 0), UDim2.new(0, 0, 0, 0), "作者：deadline3652/Cameron", BrickColor.new("Royal purple").Color, Color3.new(0, 0, 0), .4)
-- 穿墙
local noclipF = makeBaseDragUI(UDim2.new(.2, 0, .45, 0), UDim2.new(.65, 0, .45, 0), "穿墙 (N向下 M向上)")
local nobutton = makeButton(noclipF, UDim2.new(1, 0, 1, 0), UDim2.new(0, 0, 0, 0), "穿墙 关闭", BrickColor.new("Royal purple").Color, Color3.new(0, 0, 0), .4)
...
-- 循环运行服务
print("脚本运行中！作者：deadline3652/Cameron")
